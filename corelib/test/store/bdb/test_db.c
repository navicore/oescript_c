...
const static char *BYTES = "nonesensenonesensenonesensenonesensenonesensenonesensenonesensenonesensenonesensenonesense";
//const static char *BYTES = "nonesense";
...
void testIdGen(void) {
    ...
    //int csz;
    //oedb->seq->get_cachesize(oedb->seq, &csz);
    //assert(csz >= 10);
    oe_id id1 = OeStore_create_id(store);
    OeStore_create_id(store);
    OeStore_create_id(store);
    OeStore_create_id(store);
    OeStore_create_id(store);
    oe_id id2 = OeStore_create_id(store);
    assert(id2 - id1 == 5);
}
...
void testCommitPut(void) {
    ...
    assert(DataObject_get_nattrs(data) == 2);

    int ret = OeStore_put( store, holder, arena);
    if (ret) printf("testCommitPut err: %i\n", ret);
...
void testGetLarge(void) {
    ...
    assert(strcmp(DataObject_get_bytes(data), BYTES) == 0);
}
...
void testTakeLarge(void) {
    ...
    Iterator iter = DataObject_iterator(data, true);
    assert(strcmp(Iterator_next(iter), "large1") == 0);
    assert(strcmp(Iterator_next(iter), BYTES) == 0);
}
