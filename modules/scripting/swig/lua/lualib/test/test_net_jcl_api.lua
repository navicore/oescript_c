...
l2 = myclient:writeArray(array1, 9900)
l2_lid = l2:id()
...
------- test read ---------
template1 = {name="mee"}
foundme = myclient:readTable(template1);
assert(foundme)
for r in foundme do
	for k,v in pairs(r) do
	    print("got myclient: ", c, "got v: ", v)
        assert(r["name"] == "mee")
...
notfoundme = myclient:readTable({miss="ed"},502);
assert(not notfoundme())

------- test lease cancel  ---------
canfindme = myclient:readTable(template1);
...
cannotfindme = myclient:readTable(template1,1)

shouldbenil = cannotfindme()
assert(shouldbenil == nil)

lease2 = myclient:writeTable(td1, 5000) --put a new one back for tests below
lease2:id()

------- test take ---------
tookme = myclient:takeTable(template1,5003);
assert(tookme()["name"] == "mee")
gone = myclient:readTable(template1,50);
assert(gone() == nil)
...
myclient:writeTable(w1, 60000)
wt = {name="walter"}
w2 = myclient:readTable(wt, 100)
assert(w2() == nil)
txn1:rollback()
--myclient:commit(tid)
w2 = myclient:readTable(wt, 100)
...
myclient:writeTable(r1, 60000)
t2 = myclient:readTable(rtmp, 100)
assert(t2() == nil)

txn2:commit()
print("committed txn", txn2:status())
t3 = myclient:readTable(rtmp, 5005)()
assert(t3 ~= nil)
name = t3["name"]
assert(name == "omar")
print("name:", name)
...
--[[
--insane segfault if script is too big.  must be a mem bug somewhere that is triggered
--by it but perl and python and c can not recreate this issue :|
